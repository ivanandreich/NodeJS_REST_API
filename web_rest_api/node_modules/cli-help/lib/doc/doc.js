...
var HelpDocument = function(program) {
  ...
  if(this.colon === undefined) {
    this.colon = typeof(this.conf.colon) === 'boolean' ? this.conf.colon : true;
  }
...
HelpDocument.prototype.getOptionMetrics = function(keys, target) {
  ...
  for(i = 0;i < keys.length;i++) {
    ...
    for(j = 0;j < names.length;j++) {
      ...
      if(!metrics.long) {
        metrics.long = /^--/.test(name);
      }
      if(metrics.short && metrics.long) {
        metrics.both = true;
      }
    }
  }
