var pth = require('path')
  ...
  , ArgumentTypeError = require('./argument-type-error');
...
/**
 *  Combines file tests (seee file()) with URL validation.
 *
 *  This allows an argument to be either a local file or remote URL.
 *
 ...
 *  @param fields Array of URL field names that must exist on the parsed
 *  URL object.
 */
function resource(expr, resolve, extensions, opts) {
  opts = opts || {};
  var fields = opts.fields || ['protocol']
    , slashes = opts.slashes !== undefined
      ? opts.slashes : true
    , query = opts.query !== undefined
      ? opts.query : true;
  return function resource(value, arg, index) {
    var result = uri.parse(value, slashes, query)
      , isValidUrl = true
      , isValidFile = true
      , i;
    ...
    if(opts.url && isValidUrl) return result;

    var ftest = file(expr, resolve, extensions), ferr;
    try{
      value = ftest.call(this, value, arg, index);
    }catch(e) {
      isValidFile = false;
      ferr = e;
    }
    ...
    //throw new ArgumentTypeError(
      //'file or url expected for %s', [arg.toString(null)]);
    //return value;
  }
}
    ...
    function parse(value) {
      ...
      for(i = 0;i < tests.length;i++) {
        ...
        if(!result) {
          ...
          msg = scope.errors[key].message;
          throw new ArgumentTypeError(msg, parameters, scope.errors[key].code);
        }
      }
